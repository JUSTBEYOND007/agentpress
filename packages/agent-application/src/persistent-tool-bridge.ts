...
export class PersistentToolBridge implements RuntimeToolFactory {
  ...
  public async createForRun(
    ...
    return definitions
      ...
      .map((definition) => ({
        ...
        onPreflightFailure: async (arguments_, context) => {
          await this.options.toolCalls.recordPreflightFailure({
            runId,
  ...
  private async authorize(runId: string) {
    ...
    const skillRows = await this.options.database
      ...
      .leftJoin(skillRevisions, eq(skillRevisions.id, runSkillBindings.skillRevisionId))
      .where(eq(runSkillBindings.runId, runId));
    if (
      skillRows.some(
        ({ bindingHash, revisionHash, revisionId }) =>
          revisionId === null || bindingHash !== revisionHash,
      )
    ) {
      throw new ToolCallApplicationError(
        'unauthorized_tool',
        'Agent Run Skill binding is missing or does not match its pinned revision',
      );
    }
