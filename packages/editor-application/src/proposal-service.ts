...
/**
 * The public tool accepts block operations, while document mode is allowed to
 * receive a serialized ProseMirror document from the model. Normalize that
 * document at the application boundary so the editor-patch domain only ever
 * sees valid top-level blocks.
 */
function normalizeOperations(
  ...
  if (!documentOperation) {
    if (
      operations.some(
        (operation) => 'block' in operation && operation.block.type === 'doc',
      )
    ) {
      throw new EditorApplicationError(
        'invalid_batch',
  ...
  if (!Array.isArray(content) || content.length === 0 || content.length > 200) {
    throw new EditorApplicationError('invalid_batch', 'Document content must contain 1 to 200 blocks');
  }
  const nextBlocks = content.map((value) => {
    if (!isEditorBlock(value) || value.type === 'doc') {
      throw new EditorApplicationError('invalid_batch', 'Document content contains an invalid block');
    }
    return value;
  });
  ...
  for (const block of nextBlocks) {
    if (blockIds.has(block.attrs.blockId)) {
      throw new EditorApplicationError('invalid_batch', `Duplicate document block ${block.attrs.blockId}`);
    }
