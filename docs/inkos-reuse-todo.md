...
- [ ] 审计详情展示 server/tool/revision、参数摘要、耗时、重试、输出引用和脱敏错误；凭据、原始 header、
      ...
      fact 提取并限制 ToolCall `durationMs`。schema-aware 参数摘要已完成：`tool-runtime` 从受控 TypeBox
...
- [x] 明确展示“用户显式选择、模型自主选择、被策略禁用、加载失败、历史已过期”五种不同状态。
      ...
      将前者标为“仅用户显式选择”、后者标为“加载失败，无法绑定”；模型预选现在有 15 秒 host deadline，
      timeout/provider failure 以 `SkillSelectionError` 区分，并在绑定事实中保留 `modelSelection.status=failed`，
      继续使用显式 Skill；历史过期的消费者状态仍待补齐。
