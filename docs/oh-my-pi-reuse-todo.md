...
- [ ] 复用 Markdown Prompt 模板、变量渲染和小块组合模式，不复制 Coding Agent 巨型 System Prompt。
- [ ] 将角色/领域语义保留在 Prompt，将权限、路由、终态、恢复、幂等和 Schema 放在确定性代码中。
- [ ] Prompt revision 继续持久化并绑定 Context Pack；Run 历史不得使用当前 Prompt 冒充历史版本。
