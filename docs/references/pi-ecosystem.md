...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client/session/transport                 | `packages/mcp-runtime/src/client-gateway.ts`, `streamable-http.ts`, `server-manager.ts`                                                                                                                        | Official SDK `1.30.0` owns MCP client/session and JSON-RPC transport types. The host proves dispatch boundaries: exhausted connection probes before dispatch produce retryable `connection_unavailable_before_dispatch`; connection loss after dispatch and stale client results produce non-replayable `connection_lost_after_dispatch`/`stale_client_result` outcome-unknown reasons. No SDK type crosses Domain. | `packages/mcp-runtime/test/client-gateway.test.ts`, `server-manager.test.ts`, `streamable-http.integration.test.ts`                                                         |
...
| Schema/output guard                      | `packages/mcp-runtime/src/output-guard.ts`                                                                                                                                                                     | `pi-mcp-adapter` behavior is adapted behind the gateway. Guarded output is untrusted, size-bounded and optionally stored as an Artifact; JSON-RPC errors and raw payloads are translated to typed application failures.                                                                                                                                                                                             | `packages/mcp-runtime/test/output-guard.test.ts`, `built-in-tools.test.ts`                                                                                                  |
| Host Tool Registry                       | `packages/tool-runtime/src/tool-registry.ts`                                                                                                                                                                   | Registry owns tool id/version, capability, risk, timeout, input/output schema, transport provenance and evidence provider revision. MCP only supplies an adapter-backed implementation; it does not create a second executor.                                                                                                                                                                                       | `packages/tool-runtime/test/tool-registry.test.ts`, `packages/mcp-runtime/src/built-in-tools.ts`                                                                            |
...
| Persistent execution bridge              | `packages/agent-application/src/persistent-tool-bridge.ts`                                                                                                                                                     | Every MCP call enters the same `ToolCallService` proposal/approval/execute path as first-party tools. Specialist `taskId/taskAttempt` is attached here; Skills only narrow the already-authorized host set.                                                                                                                                                                                                         | `packages/agent-application/test/tool-call.integration.test.ts`, `persistent-tool-bridge.ts`                                                                                |
| ToolCall ledger and approval             | `packages/agent-application/src/tool-call-service.ts`, `tool-call-execution-service.ts`; PostgreSQL `tool_calls` and `run_events` schema                                                                       | PostgreSQL is the authority for idempotency, approval, attempt fencing, provenance drift and terminal status. MCP transport errors never become Domain state directly.                                                                                                                                                                                                                                              | `packages/agent-application/test/tool-call.integration.test.ts`, `tool-call-failure.test.ts`                                                                                |
...
| RunEvent and consumer projection         | `packages/agent-application/src/run-projection.ts`, `run-projection-service.ts`; Web `apps/web/src/lib/agent-execution-projection.ts`, `agent-tool-failure-projection.ts` and `agent-tool-audit-projection.ts` | Product projection exposes goal/result/outcome plus a bounded `outcomeReason`, never raw provider error text. Web validates the reason against the host contract and renders distinct before-dispatch, after-dispatch and stale-result guidance. Server/tool/revision/transport and raw guarded payload remain audit-only; live and replay use the same facts.                                                      | `packages/agent-application/test/run-projection.test.ts`, `tool-call-failure.test.ts`, `tool-call.integration.test.ts`; Web failure/runtime/audit projection tests          |

The map is complete for the documented local boundaries. The separate MCP completion gate remains open
until official SDK contract tests, a real MCP Server, PostgreSQL replay/reconnect proof, browser
projection checks and a real Pi target-model run all pass together.
