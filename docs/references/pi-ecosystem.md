...
- Do not copy InkOS's per-pipeline cards, default-expanded operation results, local preference state, or raw execution logs into the consumer transcript. AgentPress exposes one progressively disclosed process view derived from its durable projection and keeps approvals, failures, recovery, and reviewable outcomes visible.
