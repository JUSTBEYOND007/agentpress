...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client/session/transport                 | `packages/mcp-runtime/src/client-gateway.ts`, `streamable-http.ts`, `server-manager.ts`, `transport-errors.ts`                                                                                                 | Official SDK `1.30.0` owns MCP client/session and JSON-RPC transport types. The host proves dispatch boundaries: exhausted connection probes before dispatch are retryable; connection loss, timeout and stale results after dispatch become non-replayable typed unknown outcomes. A shared narrow classifier recognizes the SDK's typed HTTP 404 and official `-32000` session-state HTTP 400 responses for an expired Streamable HTTP session while keeping authentication, rate-limit, generic bad-request and 5xx failures independent. Safe `listTools` and frozen capability checks reconnect once before dispatch; the persisted ToolCall still invokes the provider exactly once. A stale session after `tools/call` remains non-replayable unknown and only the next logical ToolCall reconnects. Real fixtures additionally prove closed-TCP probing, pooled-socket restart recovery, initialize-only request counts, absent/expired Authorization behavior, timeout cancellation, late-result fencing and session health. No SDK type crosses Domain. | `packages/mcp-runtime/test/client-gateway.test.ts`, `server-manager.test.ts`, `streamable-http.integration.test.ts`, `packages/runtime-tools/test/mcp-postgres.integration.test.ts`                                        |
| Server capability and tool-list revision | `packages/mcp-runtime/src/built-in-tools.ts`, `in-memory-built-ins.ts`, `tool-capability.ts`                                                                                                                   | Built-in server id/revision and tool schema are converted to the host `ToolRegistry` definition. A Run snapshots `serverRevision`, `toolRevision` and `adapterRevision` in `tool_calls.transport_provenance`; remote Server or tool-list data cannot grant capability. Before dispatch, the adapter uses the official SDK `tools/list` result to require the pinned tool name and a normalized host input contract. `_agentpressRunId` is the only adapter-owned field removed before comparison; annotations and root `additionalProperties` do not affect the host arguments actually sent. Structural equality reuses MIT `fast-deep-equal@3.1.3`. Missing tools and incompatible Schema revisions fail as `tool_unavailable` without `tools/call`.                                                                                                                                                                                                                                                                                                            | `packages/mcp-runtime/test/client-gateway.test.ts`, `streamable-http.integration.test.ts`, `packages/runtime-tools/test/mcp-postgres.integration.test.ts`, `packages/agent-application/test/tool-call.integration.test.ts` |
