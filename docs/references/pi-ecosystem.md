...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client/session/transport                 | `packages/mcp-runtime/src/client-gateway.ts`, `streamable-http.ts`, `server-manager.ts`                                                                                                                        | Official SDK `1.30.0` owns MCP client/session and JSON-RPC transport types. The host proves dispatch boundaries: exhausted connection probes before dispatch are retryable; connection loss, timeout and stale results after dispatch become non-replayable typed unknown outcomes. Typed HTTP 429 is converted at this adapter boundary to a redacted, retryable host failure without degrading the client; response text and non-429 message keywords cannot trigger that classification. Real HTTP fixtures prove timeout cancellation, late-result fencing, post-429 session health and opposite semantics. No SDK type crosses Domain. | `packages/mcp-runtime/test/client-gateway.test.ts`, `server-manager.test.ts`, `streamable-http.integration.test.ts`                                                         |
...
| RunEvent and consumer projection         | `packages/agent-application/src/run-projection.ts`, `run-projection-service.ts`; Web `apps/web/src/lib/agent-execution-projection.ts`, `agent-tool-failure-projection.ts` and `agent-tool-audit-projection.ts` | Product projection exposes goal/result/outcome plus bounded typed failures, never raw provider error text. Rate limiting is the exact `tool_rate_limited`/`tool.failure.rate_limited`/retryable contract and remains ordinary `failed`; Web rejects mismatched fields. Transport reasons retain distinct before-dispatch, after-dispatch and stale-result guidance. Server/tool/revision/transport and raw guarded payload remain audit-only; live and replay use the same facts.                                                                                                                                                           | `packages/agent-application/test/run-projection.test.ts`, `tool-call-failure.test.ts`, `tool-call.integration.test.ts`; Web failure/runtime/audit projection tests          |
