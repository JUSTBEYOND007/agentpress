...
| 正文选区 | `选中 2 个段落 ×`  | 仅属于当前文章，绑定稳定 blockId/hash；失效时发送前提示 |
...
- 其他文章只能作为 `@文章` 参考上下文，不能作为当前文章正文修改工具的选区或写入目标。
...
正文修改 Run 采用严格 revision 快照：Run 启动后正文 revision 发生变化时，不重放或自动变基旧提案，必须基于最新正文重新运行。
